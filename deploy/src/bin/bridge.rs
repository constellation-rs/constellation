...
use std::{net,fs,io,env,ffi,thread,sync,path,mem,os};
use std::sync::mpsc;
...
use deploy_common::{Pid,Resources,ProcessOutputEvent,ProcessInputEvent,DeployOutputEvent,DeployInputEvent,Signal,copy,BufferedStream,map_bincode_err,move_fds,FdIter,copy_sendfile};
...
fn parse_request<R: Read>(mut stream: &mut R) -> Result<(Option<Resources>,Vec<ffi::OsString>,Vec<(ffi::OsString,ffi::OsString)>,fs::File,Vec<u8>),io::Error> {
...
fn monitor_process(pid: Pid, sender_: mpsc::SyncSender<OutputEventInt>, receiver_: mpsc::Receiver<InputEventInt>) {
	...
	thread::spawn(move||{
		...
		let x = PROCESS_COUNT.fetch_sub(1, sync::atomic::Ordering::Relaxed); assert_ne!(x, 0);
		logln!("BRIDGE: KILL ({})", x);
	});
	loop {
		...
		match event {//.unwrap() {
			ProcessOutputEvent::Spawn(new_pid) => {
				...
				sender_.send(OutputEventInt::Spawn(pid, new_pid, sender1)).unwrap();
				let sender_ = sender_.clone();
				thread::Builder::new().name(String::from("d")).spawn(move||{
					monitor_process(new_pid, sender_, receiver1);
				}).unwrap();
			}
			ProcessOutputEvent::Output(fd, output) => {
				sender_.send(OutputEventInt::Output(pid, fd, output)).unwrap();
			}
...
fn recce(elf: &fs::File, args: &Vec<ffi::OsString>, vars: &Vec<(ffi::OsString,ffi::OsString)>) -> Result<Resources,()> {
	let (reader, writer) = nix::unistd::pipe2(nix::fcntl::OFlag::empty()).unwrap();
	let child = if let nix::unistd::ForkResult::Parent{child, ..} = nix::unistd::fork().expect("Fork failed") {child} else {
		// println!("{:?}", args[0]);
		let err = unsafe{nix::libc::prctl(nix::libc::PR_SET_PDEATHSIG, nix::libc::SIGKILL)}; assert_eq!(err, 0);
		let vars = iter::once((ffi::CString::new("DEPLOY").unwrap(),ffi::CString::new("fabric").unwrap())).chain(iter::once((ffi::CString::new("DEPLOY_RECCE").unwrap(),ffi::CString::new("1").unwrap()))).chain(vars.into_iter().map(|(x,y)|(ffi::CString::new(unix::ffi::OsStringExt::into_vec(x.clone())).unwrap(),ffi::CString::new(unix::ffi::OsStringExt::into_vec(y.clone())).unwrap()))).map(|(key,value)|ffi::CString::new(format!("{}={}",key.to_str().unwrap(),value.to_str().unwrap())).unwrap()).collect::<Vec<_>>();
		...
		if false {
			nix::unistd::execve(&ffi::CString::new(unix::ffi::OsStringExt::into_vec(args[0].clone())).unwrap(), &args.into_iter().map(|x|ffi::CString::new(unix::ffi::OsStringExt::into_vec(x.clone())).unwrap()).collect::<Vec<_>>(), &vars).expect("Failed to fexecve ELF");
		} else {
// 			if is_valgrind() {
...
// 			}
			nix::unistd::fexecve(4, &args.into_iter().map(|x|ffi::CString::new(unix::ffi::OsStringExt::into_vec(x.clone())).unwrap()).collect::<Vec<_>>(), &vars).expect("Failed to fexecve ELF");
		}
		unreachable!();
	};
	...
	bincode::deserialize_from(&mut &reader).map_err(map_bincode_err).map_err(|_|())
}
...
fn main() {
	...
	thread::Builder::new().name(String::from("a")).spawn(move||{
		for stream in listener.incoming() {
			logln!("BRIDGE: accepted");
			let mut stream = stream.unwrap();
			let sender = sender.clone();
			thread::Builder::new().name(String::from("b")).spawn(move||{
				let stream = stream;
				let (mut stream_read,mut stream_write) = (BufferedStream::new(&stream),&stream);
				if let Ok((process,args,vars,elf,mut arg)) = parse_request(&mut stream_read) {
					assert_eq!(arg.len(), 0);
					bincode::serialize_into(&mut arg, &deploy::pid()).unwrap();
					let (sender_, receiver) = mpsc::sync_channel::<_>(0);
					sender.send((process.unwrap_or_else(||recce(&elf, &args, &vars).unwrap()),args,vars,elf,arg,sender_)).unwrap();
					...
					if let Some(pid) = pid {
						...
						let (sender, receiver) = mpsc::sync_channel::<_>(0);
						let (sender1, receiver1) = mpsc::sync_channel::<_>(0);
						thread::Builder::new().name(String::from("c")).spawn(move||{
								...
								loop {
									...
									match event.unwrap() {
										DeployInputEvent::Input(pid, fd, input) => {
											hashmap.lock().unwrap().get(&pid).unwrap().send(InputEventInt::Input(fd, input)).unwrap();
										}
										DeployInputEvent::Kill(Some(pid)) => {
											hashmap.lock().unwrap().get(&pid).unwrap().send(InputEventInt::Kill).unwrap();
										}
							...
							for event in receiver.iter() {
								let event = match event {
									OutputEventInt::Spawn(pid, new_pid, sender) => {
										let x = hashmap.lock().unwrap().insert(new_pid, sender); assert!(x.is_none());
										DeployOutputEvent::Spawn(pid, new_pid)
									}
									OutputEventInt::Output(pid, fd, output) => DeployOutputEvent::Output(pid, fd, output),
									OutputEventInt::Exit(pid, exit_code) => {
										hashmap.lock().unwrap().remove(&pid).unwrap();
										DeployOutputEvent::Exit(pid, exit_code)
									}
								};
								if let Err(_) = bincode::serialize_into(&mut stream_write, &event) {
									break;
								}
							}
						...
						assert_eq!(hashmap.lock().unwrap().len(), 0, "{:?}", *hashmap.lock().unwrap());
					}
				}
			}).unwrap();
		}
	...
	for (process,args,vars,elf,arg,sender) in receiver {
		...
		let (mut scheduler_read,mut scheduler_write) = (BufferedStream::new(&scheduler),BufferedStream::new(&scheduler));
		...
		let pid: Option<Pid> = bincode::deserialize_from(&mut scheduler_read).map_err(map_bincode_err).unwrap();
