...
use std::{fmt,net,thread,ops,marker,sync,os,mem,ptr,error,cell};
use std::collections::{HashMap};
use std::sync::Arc;

// use self::circularbuffer::{Readable,Writable};
use self::tcp_linux::{EpollExecutor,EpollExecutorContext,Triggerer,/*socket_forwarder,SocketForwarder,SocketForwardee,*/LinuxTcpListener,LinuxTcp};
pub use self::tcp_linux::{socket_forwarder,SocketForwarder,SocketForwardee};
use either::Either;
...
impl ops::Drop for Handle {
	fn drop(&mut self) {
		...
		logln!("{:?}: {:?}: self.tcp_thread.take().unwrap().join().unwrap()", nix::unistd::getpid(), ::pid());
...
impl Context {
	pub fn new(host: net::IpAddr) -> (Context,u16) {
		...
		let (listener,port) = LinuxTcpListener::new_ephemeral(&host, &executor.context(ptr::null()));
		let sockets = sync::RwLock::new(HashMap::new());
		let local = net::SocketAddr::new(host, port);
		(Context{executor,listener:sync::RwLock::new(Some(listener)),sockets,local,forward_listener:None},port)
	}
	pub fn with_fd(fd: os::unix::io::RawFd, forward_listener: Option<SocketForwarder>) -> Context {
		...
		let sockets = sync::RwLock::new(HashMap::new());
		let local = if let nix::sys::socket::SockAddr::Inet(inet) = nix::sys::socket::getsockname(fd).unwrap() {inet.to_std()} else {panic!()};
		Context{executor,listener:sync::RwLock::new(Some(listener)),sockets,local,forward_listener}
	}
	pub fn with_socket_forwardee(socket_forwardee: SocketForwardee, local: net::SocketAddr) -> Context {
		// logln!("printsockets: {:?}", (printsockets as extern "C" fn(&HashMap<net::SocketAddr,Arc<sync::RwLock<Option<Channel>>>>)) as *const ());
		let executor = EpollExecutor::new();
		let listener = LinuxTcpListener::with_socket_forwardee(socket_forwardee, &executor.context(ptr::null()));
		let sockets = sync::RwLock::new(HashMap::new());
		Context{executor,listener:sync::RwLock::new(Some(listener)),sockets,local,forward_listener:None}
	}
	pub fn tcp<F:FnMut(&os::unix::io::RawFd)->Option<SocketForwarder>+marker::Send+'static>(context: &'static sync::RwLock<Option<Context>>, mut accept_hook: F) -> Handle {
		let (triggerer,triggeree) = {let context = context.try_read().unwrap(); context.as_ref().unwrap().executor.context(1 as *const ()).add_trigger()};
		let tcp_thread = thread::Builder::new().name("tcp-thread".to_string()).spawn(move||{
			let context = context.try_read().unwrap();
			let context = context.as_ref().unwrap();
			let mut xxx = context.listener.try_write().unwrap();
			let (executor,listener,sockets,local) = (&context.executor,xxx.as_mut().unwrap(),&context.sockets,&context.local);
			let mut done: Option<sync::RwLockWriteGuard<HashMap<net::SocketAddr,Arc<sync::RwLock<Option<Channel>>>>>> = None;
			while done.is_none() || done.as_ref().unwrap().iter().filter(|&(_,ref inner)|{ // TODO: maintain count
				...
				if let &Some(ref socket) = &done {
					logln!("{:?}: {:?}: socket: {:?}", nix::unistd::getpid(), ::pid(), &**socket);
				}
				executor.wait(|executor, _events, data|{
					if data == ptr::null() {
						for (connectee,remote,executor_hack) in listener.poll(&executor.context(ptr::null()), &mut accept_hook) {
							assert!(!ord(&local, &remote));
							let is_done = done.is_some();
							let mut sockets_ = if done.is_none() {Some(sockets.write().unwrap())} else {None};
							let sockets = done.as_mut().map(|x|&mut**x).unwrap_or_else(||&mut*sockets_.as_mut().unwrap());
							if sockets.contains_key(&remote) {
								let channel_ = &**sockets.get(&remote).unwrap();
								if let &Inner::Connected(ref e) = &channel_.read().unwrap().as_ref().unwrap().inner {
								...
								let executor = &executor.context(executor_key as *const ());
								executor_hack(executor, &connectee);
								let mut inner = Inner::connect(local.clone(), remote, Some(connectee), executor);
								if is_done {
									if inner.closable() {
								...
								if !inner.closed() {
									*channel.try_write().unwrap() = Some(Channel::new(inner));
									let x = sockets.insert(remote, channel); assert!(x.is_none());
								}
							}
						}
						...
						if done.is_none() {
							...
							if let Some(channel_arc) = channel_arc { // TODO
								let mut channel = channel_arc.write().unwrap();
								assert_eq!(sync::Arc::strong_count(&channel_arc), 1+channel.as_ref().unwrap().senders_count+channel.as_ref().unwrap().receivers_count, "{:?}:{:?} {:?}", getpid(), gettid(), executor_key);
								if {
									...
									channel.senders_count == 0 && channel.receivers_count == 0 && inner.closed()
								} {
									let x = channel.take().unwrap(); assert!(x.senders_count == 0 && x.receivers_count == 0 && x.inner.closed());
									let key = sockets.iter().find(|&(_key,channel)|{
										let executor_key2: *const sync::RwLock<Option<Channel>> = &**channel;
										executor_key2 == executor_key
									}).unwrap().0.clone();
									mem::drop(channel);
									logln!("{:?}:{:?} drop epoll1 {:?}", getpid(), gettid(), executor_key);
									let mut x = Arc::try_unwrap(sockets.remove(&key).unwrap()).expect("channel.try_unwrap epoll 1"); assert!(x.get_mut().unwrap().is_none());
							...
							if let Some(channel_arc) = channel_arc { // TODO
								...
								if {
									...
									mem::drop(channel);
									logln!("{:?}:{:?} drop epoll2 {:?}", getpid(), gettid(), executor_key);
									let mut x = Arc::try_unwrap(sockets.remove(&key).unwrap()).expect("channel.try_unwrap epoll 2"); assert!(x.get_mut().unwrap().is_none());
									logln!("channel.try_unwrap epoll 2 success");
								}
							}
						}
					} else { assert!(done.is_none());
		...
		Handle{trigger:triggerer,tcp_thread:Some(tcp_thread)}
	}
}
impl ops::Drop for Context {
	fn drop(&mut self) {
		...
		if let &Some(ref forward_listener) = &self.forward_listener {
			...
			forward_listener.send(self.listener.try_write().unwrap().take().unwrap().into_fd()).unwrap();
		}
	}
}
...
impl Channel {
	fn new(inner: Inner) -> Channel {
		Channel{inner, senders_count:0, receivers_count:0, senders:HashMap::new(), senders_futures:Vec::new(), receivers:HashMap::new(), receivers_futures:Vec::new()}
	}
}
...
impl Inner {
	pub fn connect(local: net::SocketAddr, remote: net::SocketAddr, incoming: Option<LinuxTcp>, executor: &EpollExecutorContext) -> Self {
	...
	pub fn poll(&mut self, executor: &EpollExecutorContext) {
		*self = match mem::replace(self, Inner::Xxx) {
			...
			Inner::ConnectingLocalClosed(connecting_local_closed) => match connecting_local_closed.poll(executor) {
				Either::Left(connecting_local_closed) => Inner::ConnectingLocalClosed(connecting_local_closed),
				Either::Right(Some(Either::Left(connected))) => Inner::LocalClosed(connected),
				Either::Right(Some(Either::Right(Either::Left(remote_closed)))) => Inner::Closing(remote_closed),
	...
	pub fn recv_avail<T:serde::de::DeserializeOwned+'static>(&mut self, executor: &EpollExecutorContext) -> bool {
		self.poll(executor);
		match self {
	...
	pub fn recv<T:serde::de::DeserializeOwned+'static>(&mut self, executor: &EpollExecutorContext) -> T {
		let ret = match self {
	...
	pub fn send<T:serde::ser::Serialize+'static>(&mut self, x: T, executor: &EpollExecutorContext) {
		match self {
	...
	pub fn close(&mut self, executor: &EpollExecutorContext) {
		*self = match mem::replace(self, Inner::Xxx) {
			Inner::Connecting(connecting) => match connecting.close(executor) {
				...
				Either::Right(Some(Either::Right(Either::Left(closing)))) => Inner::Closing(closing),
				Either::Right(Some(Either::Right(Either::Right(())))) => Inner::Closed,
				Either::Right(None) => Inner::Killed,
			}
...
fn ord(a: &net::SocketAddr, b: &net::SocketAddr) -> bool { // TODO: cipher?
	assert_ne!(a, b);
	if a.ip() > b.ip() {
		return true;
	}
...
impl InnerConnecting {
	fn new(local: net::SocketAddr, remote: net::SocketAddr, incoming: Option<LinuxTcp>, executor: &EpollExecutorContext) -> Either<Self,Option<Either<InnerConnected,InnerConnectedRemoteClosed>>> {
		if ord(&local, &remote) {
			InnerConnecting{outgoing:Some(LinuxTcp::connect(local, remote, executor)),incoming}
		} else {
			InnerConnecting{outgoing:None,incoming}
		}.poll(executor)
	}
	fn poll(mut self, executor: &EpollExecutorContext) -> Either<Self,Option<Either<InnerConnected,InnerConnectedRemoteClosed>>> {
		if self.incoming.is_some() {
			self.incoming.as_mut().unwrap().poll(executor);
			if !self.incoming.as_ref().unwrap().connecting() {
				return Either::Right(match InnerConnected::new(self.incoming.take().unwrap(), executor) {
		...
		if self.outgoing.is_some() {
			...
			if !self.outgoing.as_ref().unwrap().connecting() {
				return Either::Right(match InnerConnected::new(self.outgoing.take().unwrap(), executor) {
					Either::Left(connected) => Some(Either::Left(connected)),
					Either::Right(Some(remote_closed)) => Some(Either::Right(remote_closed)),
					Either::Right(None) => None,
				});
	...
	fn close(self, executor: &EpollExecutorContext) -> Either<InnerConnectingLocalClosed,Option<Either<InnerConnectedLocalClosed,Either<InnerConnectedClosing,()>>>> {
...
impl InnerConnectingLocalClosed {
	fn new(outgoing: Option<LinuxTcp>, incoming: Option<LinuxTcp>, executor: &EpollExecutorContext) -> Either<Self,Option<Either<InnerConnectedLocalClosed,Either<InnerConnectedClosing,()>>>> {
		InnerConnectingLocalClosed{outgoing,incoming}.poll(executor)
	}
	fn poll(mut self, executor: &EpollExecutorContext) -> Either<Self,Option<Either<InnerConnectedLocalClosed,Either<InnerConnectedClosing,()>>>> {
		if self.incoming.is_some() {
			self.incoming.as_mut().unwrap().poll(executor);
			...
			if !self.incoming.as_ref().unwrap().connecting() {
				return Either::Right(match InnerConnectedLocalClosed::new(self.incoming.take().unwrap(), serialize::Serializer::new(), serialize::Deserializer::new(), false, executor) {
					Either::Left(connected) => Some(Either::Left(connected)),
					Either::Right(Some(remote_closed)) => Some(Either::Right(remote_closed)),
					Either::Right(None) => None,
				});
		...
		if self.outgoing.is_some() {
			...
			if !self.outgoing.as_ref().unwrap().connecting() {
				return Either::Right(match InnerConnectedLocalClosed::new(self.outgoing.take().unwrap(), serialize::Serializer::new(), serialize::Deserializer::new(), false, executor) {
					Either::Left(connected) => Some(Either::Left(connected)),
					Either::Right(Some(remote_closed)) => Some(Either::Right(remote_closed)),
					Either::Right(None) => None,
				});
...
impl InnerConnected {
	fn new(connection: LinuxTcp, executor: &EpollExecutorContext) -> Either<Self,Option<InnerConnectedRemoteClosed>> {
		InnerConnected{connection,send_serializer:serialize::Serializer::new(),recv_deserializer:serialize::Deserializer::new(),recv_deserializer_given:false}.poll(executor)
	}
	fn poll(mut self, executor: &EpollExecutorContext) -> Either<Self,Option<InnerConnectedRemoteClosed>> {
		...
		// }
		if !self.connection.recvable() && !self.recv_deserializer.mid() && !self.recv_deserializer.pull2_avail() { // self.recv_deserializer.pull_avail() {
			// assert!(!self.recv_deserializer_given);
			return Either::Right(match InnerConnectedRemoteClosed::new(self.connection, self.send_serializer, executor) {
	...
	fn recv_avail<T:serde::de::DeserializeOwned+'static>(&mut self, executor: &EpollExecutorContext) -> bool {
	...
	fn recv<T:serde::de::DeserializeOwned+'static>(&mut self, executor: &EpollExecutorContext) -> T {
	...
	fn close(self, executor: &EpollExecutorContext) -> Either<InnerConnectedLocalClosed,Option<Either<InnerConnectedClosing,()>>> {
		InnerConnectedLocalClosed::new(self.connection, self.send_serializer, self.recv_deserializer, self.recv_deserializer_given, executor)
	}
}
...
impl InnerConnectedRemoteClosed {
	fn new(connection: LinuxTcp, send_serializer: serialize::Serializer, executor: &EpollExecutorContext) -> Option<Self> {
		InnerConnectedRemoteClosed{connection,send_serializer}.poll(executor)
	}
...
impl InnerConnectedLocalClosed {
	fn new(connection: LinuxTcp, send_serializer: serialize::Serializer, recv_deserializer: serialize::Deserializer, recv_deserializer_given: bool, executor: &EpollExecutorContext) -> Either<Self,Option<Either<InnerConnectedClosing,()>>> {
		InnerConnectedLocalClosed{connection,send_serializer,recv_deserializer,recv_deserializer_given}.poll(executor)
	}
	fn poll(mut self, executor: &EpollExecutorContext) -> Either<Self,Option<Either<InnerConnectedClosing,()>>> {
		assert!(!self.connection.connecting());
		let mut progress = true;
		loop {
			if self.connection.sendable() {
		...
		if !self.connection.recvable() && !self.recv_deserializer.mid() && !self.recv_deserializer.pull2_avail() { // self.recv_deserializer.pull_avail() {
			// assert!(!self.recv_deserializer_given);
			return Either::Right(match InnerConnectedClosing::new(self.connection, self.send_serializer, executor) {
...
impl InnerConnectedClosing {
	fn new(connection: LinuxTcp, send_serializer: serialize::Serializer, executor: &EpollExecutorContext) -> Either<Self,Option<()>> {
		InnerConnectedClosing{connection,send_serializer}.poll(executor)
	}
...
impl fmt::Display for ChannelError {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			ChannelError::Error => write!(f, "Remote process died or channel killed by OS/hardware"), //(ref err) => err.fmt(f),
...
impl<T:serde::ser::Serialize> Sender<T> {
	pub fn new(remote: net::SocketAddr, context: &Context) -> Option<Sender<T>> {
		...
		let channel = if !sockets.contains_key(&remote) {
			...
			*channel.try_write().unwrap() = Some(inner);
			let x = sockets.insert(remote, channel.clone()); assert!(x.is_none());
			logln!("{:?}:{:?} new sender {:?}", getpid(), gettid(), executor_key);
			channel
		} else {
			...
			let executor_key: *const sync::RwLock<Option<Channel>> = &**channel;
			logln!("{:?}:{:?} retain sender {:?}", getpid(), gettid(), executor_key);
			channel.clone()
		};
		assert_eq!(sync::Arc::strong_count(&channel), 1+{let channel = channel.read().unwrap(); channel.as_ref().unwrap().senders_count+channel.as_ref().unwrap().receivers_count});
		Some(Sender{channel:Some(channel),_marker:marker::PhantomData})
	}
	fn async_send_available(&self, context: &Context) -> bool {
		match &self.channel.as_ref().unwrap().read().unwrap().as_ref().unwrap().inner {
			&Inner::Connected(ref connected) => connected.send_avail(),
	...
	fn async_send(&self, t: T, context: &Context) -> Result<(),ChannelError> where T: 'static {
	...
	pub fn xxx_send<'a>(&'a self, context: &Context) -> Option<impl FnOnce(T,&Context)->Result<(),ChannelError>+'a> where T: 'static {
	...
	pub fn send<F:FnMut()->C,C:ops::Deref<Target=Context>>(&self, t: T, context: &mut F) -> Result<(),ChannelError> where T: 'static {
		let x = cell::RefCell::new(None);
		select(vec![Box::new(self.selectable_send(||{*x.borrow_mut() = Some(Ok(()));t},|e|*x.borrow_mut() = Some(Err(e))))], context);
		x.into_inner().unwrap()
	}
	pub fn selectable_send<'a,F:FnOnce()->T+'a,E:FnOnce(ChannelError)+'a>(&'a self, f: F, e: E) -> impl Selectable + 'a where T: 'static {
		Send(self, Some((f, e)))
	}
	pub fn drop(mut self, context: &Context) {
		...
		if {
			...
			logln!("{:?}:{:?} release sender {:?}", getpid(), gettid(), executor_key);
			...
			logln!("{:?}:{:?} drop sender {:?}", getpid(), gettid(), executor_key);
			let mut x = Arc::try_unwrap(sockets.remove(&key).unwrap()).expect("channel.try_unwrap drop 1"); assert!(x.get_mut().unwrap().is_none());
			logln!("channel.try_unwrap drop 1 success");
		}
	}
	pub fn futures_poll_ready(&mut self, cx: &mut futures::task::Context, context: &Context) -> Result<futures::Async<()>, ChannelError> where T: 'static {
		if let Some(recv) = self.xxx_send(context) {
			Ok(futures::Async::Ready(()))
		} else {
			self.channel.as_ref().unwrap().write().unwrap().as_mut().unwrap().senders_futures.push(cx.waker().clone());
			Ok(futures::Async::Pending)
		}
	}
	pub fn futures_start_send(&mut self, item: T, context: &Context) -> Result<(), ChannelError> where T: 'static {
		self.xxx_send(context).expect("called futures::Sink::start_send without the go-ahead from futures::Sink::poll_ready")(item, context)
	}
}
...
struct Send<'a,T:serde::ser::Serialize+'static,F:FnOnce()->T,E:FnOnce(ChannelError)>(&'a Sender<T>,Option<(F,E)>);
impl<'a,T:serde::ser::Serialize+'static,F:FnOnce()->T,E:FnOnce(ChannelError)> fmt::Debug for Send<'a,T,F,E> {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		f.debug_struct("Send")
		 .field("sender", &self.0)
		 .finish()
...
impl<'a,T:serde::ser::Serialize+'static,F:FnOnce()->T,E:FnOnce(ChannelError)> Selectable for Send<'a,T,F,E> {
	fn subscribe(&self, thread: thread::Thread) {
		let x = self.0.channel.as_ref().unwrap().write().unwrap().as_mut().unwrap().senders.insert(thread.id(), thread); assert!(x.is_none());
	}
	...
	fn unsubscribe(&self, thread: thread::Thread) {
		self.0.channel.as_ref().unwrap().write().unwrap().as_mut().unwrap().senders.remove(&thread.id()).unwrap();
	}
}
...
impl<T:serde::de::DeserializeOwned> Receiver<T> {
	pub fn new(remote: net::SocketAddr, context: &Context) -> Option<Receiver<T>> {
		...
		let channel = if !sockets.contains_key(&remote) {
			...
			*channel.try_write().unwrap() = Some(inner);
			let x = sockets.insert(remote, channel.clone()); assert!(x.is_none());
			logln!("{:?}:{:?} new receiver {:?}", getpid(), gettid(), executor_key);
			channel
		} else {
			let channel = sockets.get_mut(&remote).unwrap();
			...
			let executor_key: *const sync::RwLock<Option<Channel>> = &**channel;
			logln!("{:?}:{:?} retain receiver {:?}", getpid(), gettid(), executor_key);
			channel.clone()
		};
		assert_eq!(sync::Arc::strong_count(&channel), 1+{let channel = channel.read().unwrap(); channel.as_ref().unwrap().senders_count+channel.as_ref().unwrap().receivers_count});
		Some(Receiver{channel:Some(channel),_marker:marker::PhantomData})
	}
	fn async_recv_available(&self, context: &Context) -> bool where T: 'static {
		let executor = &context.executor;
		let executor_key: *const sync::RwLock<Option<Channel>> = &**self.channel.as_ref().unwrap();
		let executor = &executor.context(executor_key as *const ());
		let mut channel = self.channel.as_ref().unwrap().write().unwrap();
	...
	fn async_recv(&self, context: &Context) -> Result<T,ChannelError> where T: 'static {
		let executor = &context.executor;
		let executor_key: *const sync::RwLock<Option<Channel>> = &**self.channel.as_ref().unwrap();
		let executor = &executor.context(executor_key as *const ());
		let mut channel = self.channel.as_ref().unwrap().write().unwrap();
	...
	pub fn xxx_recv<'a>(&'a self, context: &Context) -> Option<impl FnOnce(&Context)->Result<T,ChannelError>+'a> where T: 'static {
	...
	pub fn recv<F:FnMut()->C,C:ops::Deref<Target=Context>>(&self, context: &mut F) -> Result<T,ChannelError> where T: 'static {
		let x = cell::RefCell::new(None);
		select(vec![Box::new(self.selectable_recv(|t|*x.borrow_mut() = Some(Ok(t)),|e|*x.borrow_mut() = Some(Err(e))))], context);
		x.into_inner().unwrap()
	}
	pub fn selectable_recv<'a,F:FnOnce(T)+'a,E:FnOnce(ChannelError)+'a>(&'a self, f: F, e: E) -> impl Selectable + 'a where T: 'static {
		Recv(self, Some((f, e)))
	}
	pub fn drop(mut self, context: &Context) {
		let mut sockets = context.sockets.write().unwrap();
		...
		if {
			...
			logln!("{:?}:{:?} release receiver {:?}", getpid(), gettid(), executor_key);
			channel.senders_count == 0 && channel.receivers_count == 0 && channel.inner.closed()
		} {
			let x = channel.take().unwrap(); assert!(x.senders_count == 0 && x.receivers_count == 0 && x.inner.closed());
			let key = sockets.iter().find(|&(_key,channel)|{
			...
			assert_eq!(sync::Arc::strong_count(&channel_arc), 2);
			mem::drop(channel_arc);
			logln!("{:?}:{:?} drop receiver {:?}", getpid(), gettid(), executor_key);
			let mut x = Arc::try_unwrap(sockets.remove(&key).unwrap()).expect("channel.try_unwrap drop 2"); assert!(x.get_mut().unwrap().is_none());
			logln!("channel.try_unwrap drop 2 success");
		}
	}
	pub fn futures_poll_next(&mut self, cx: &mut futures::task::Context, context: &Context) -> Result<futures::Async<Option<T>>, ChannelError> where T: 'static {
		if let Some(recv) = self.xxx_recv(context) {
			...
			self.channel.as_ref().unwrap().write().unwrap().as_mut().unwrap().receivers_futures.push(cx.waker().clone());
			Ok(futures::Async::Pending)
		}
	}
}
...
struct Recv<'a,T:serde::de::DeserializeOwned+'static,F:FnOnce(T),E:FnOnce(ChannelError)>(&'a Receiver<T>,Option<(F,E)>);
impl<'a,T:serde::de::DeserializeOwned+'static,F:FnOnce(T),E:FnOnce(ChannelError)> fmt::Debug for Recv<'a,T,F,E> {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		f.debug_struct("Recv")
		 .field("receiver", &self.0)
		 .finish()
	}
}
impl<'a,T:serde::de::DeserializeOwned+'static,F:FnOnce(T),E:FnOnce(ChannelError)> Selectable for Recv<'a,T,F,E> {
	fn subscribe(&self, thread: thread::Thread) {
		let x = self.0.channel.as_ref().unwrap().write().unwrap().as_mut().unwrap().receivers.insert(thread.id(), thread); assert!(x.is_none());
	}
	fn available(&self, context: &Context) -> bool {
	...
	fn unsubscribe(&self, thread: thread::Thread) {
		self.0.channel.as_ref().unwrap().write().unwrap().as_mut().unwrap().receivers.remove(&thread.id()).unwrap();
	}
}
...
pub trait Selectable: fmt::Debug {
	#[doc(hidden)] fn subscribe(&self, thread::Thread);
	#[doc(hidden)] fn available(&self, context: &Context) -> bool;
	#[doc(hidden)] fn run(&mut self, context: &Context);
	#[doc(hidden)] fn unsubscribe(&self, thread::Thread);
}
pub fn select<'a,F:FnMut()->C,C:ops::Deref<Target=Context>>(select: Vec<Box<Selectable + 'a>>, context: &mut F) -> impl Iterator<Item=Box<Selectable + 'a>>+'a {
