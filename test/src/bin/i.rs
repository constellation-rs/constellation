...
fn sub2<T: std::fmt::Display + for<'de> serde::de::Deserialize<'de> + serde::ser::Serialize + 'static>(parent: Pid, arg: (usize, T)) {
	println!("! sub2");
	if arg.0 != 0 {
		let child_pid = spawn(sub2, (arg.0-1,format!("hello alec! {}; {}", arg.0-1, arg.1)), Resources{mem:20*1024*1024,..Resources::default()}).expect("SPAWN FAILED");
...
fn sub3(parent: Pid, arg: usize) {
	...
	for i in 0..pids.len() {
		for j in 0..pids.len() {
			if i == arg {
				senders.push(if i != j {Some(Sender::new(pids[j]))} else { None });
			}
			if j == arg {
				receivers.push(if i != j {Some(Receiver::new(pids[i]))} else { None });
			}
		}
	} assert_eq!(senders.len(), pids.len()); assert_eq!(receivers.len(), pids.len());
	println!("done");
	for i in 0..pids.len() {
		for j in 0..pids.len() {
			if i == j {
...
fn main() {
	...
	for i in 0..a {
		let pid = spawn(sub, (String::from("HELLO!!!"),String::from("THERE!!!")), Resources{mem:20*1024*1024,..Resources::default()}).expect("SPAWN FAILED");
	...
	{
		let count = b;
		let pid = spawn(sub2, (count,format!("hello alec! {}", count)), Resources{mem:20*1024*1024,..Resources::default()}).expect("SPAWN FAILED");
	...
	{
		let count = c;
		let pids: Vec<Pid> = (0..count).map(|i|spawn(sub3, i, Resources{mem:20*1024*1024,..Resources::default()}).expect("SPAWN FAILED")).collect();
		let senders: Vec<Sender<std::vec::Vec<deploy::Pid>>> = pids.iter().map(|&pid|Sender::new(pid)).collect();
