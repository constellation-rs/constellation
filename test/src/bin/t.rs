...
extern crate deploy;
extern crate serde;
use std::{env,io,thread,time,mem};
...
fn main() {
	...
	let pids = (0..10).map(|_|spawn(sub, (), Resources{mem:20*1024*1024,..Resources::default()}).expect("SPAWN FAILED")).collect::<Vec<_>>();
	let channels = pids.iter().map(|&pid|(Sender::<String>::new(pid),Receiver::<usize>::new(pid))).collect::<Vec<_>>();
	...
	mem::drop(channels);
	thread::sleep(time::Duration::new(0,100_000_000));
	let channels = pids.iter().map(|&pid|(Sender::<usize>::new(pid),Receiver::<String>::new(pid))).collect::<Vec<_>>();
	for &(ref sender,ref receiver) in channels.iter() {
		println!("{}", receiver.recv().unwrap());
	}
	for &(ref sender,ref receiver) in channels.iter() {
