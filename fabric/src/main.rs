			...
			Some("master") => {
				...
				loop {
					match (
							...
							loop {
								match args.peek().map(|x| x.parse::<net::SocketAddr>()) {
									Some(Err(_binary)) => {
										...
										{
											(binary, Some(Ok(addr))) => run.push(Run {
												binary: PathBuf::from(binary),
												addr,
											}),
											_ => {
												println!("Invalid bridge, expecting bridge: <bridge> <addr> or worker options: <addr> <mem> <cpu>\n{}", USAGE);
												process::exit(1)
											}
										}
									}
						...
						_ => {
							println!("Invalid worker options, expecting <addr> <mem> <cpu>, like 127.0.0.1:9999 400GiB 34\n{}", USAGE);
							process::exit(1)
						}
					}
				}
...
> {
	...
	let x = nix::unistd::lseek64(arg.as_raw_fd(), 0, nix::unistd::Whence::SeekSet).unwrap();
	assert_eq!(x, 0);
			...
			crossbeam::scope(|scope| {
				scope.spawn(move || {
					...
					for (pid, done) in receiver.iter() {
						...
						if let Err(_) = bincode::serialize_into(&mut stream_write, &done)
				...
				while let Ok((resources, ports, elf, args, vars, arg)) =
					parse_request(&mut stream_read)
				{
					...
					let child = match nix::unistd::fork().expect("Fork failed") {
						nix::unistd::ForkResult::Child => {
							...
							let mut elf_desired_fd =
								BOUND_FD_START + ports.len() as std::os::unix::io::RawFd;
							...
							for (i, port) in ports.into_iter().enumerate() {
								...
								nix::sys::socket::bind(
									socket,
									&nix::sys::socket::SockAddr::Inet(
										nix::sys::socket::InetAddr::from_std(&port),
									),
								).unwrap();
							}
							let vars = iter::once((
					...
					scope.spawn(move || {
						match nix::sys::wait::waitpid(child, None).unwrap() {
							nix::sys::wait::WaitStatus::Exited(pid, code) if code == 0 => {
								assert_eq!(pid, child)
							}
