use std::{net,fs,io,path,env,thread,ffi,mem};
use std::sync::mpsc;
...
use deploy_common::{Resources,copy,BufferedStream,Pid,PidInternal,map_bincode_err};
...
impl Node {
	...
	fn alloc(&mut self, process: &Resources) { assert!(process.cpu <= self.cpu);
...
fn parse_request<R: Read>(mut stream: &mut R) -> Result<(Resources,Vec<ffi::OsString>,Vec<(ffi::OsString,ffi::OsString)>,Vec<u8>,Vec<u8>),io::Error> {
...
pub fn run(addr: net::SocketAddr, nodes: HashMap<net::SocketAddr,(u64,f32,Vec<(path::PathBuf,Vec<net::SocketAddr>)>)>) {
	let (sender, receiver) = mpsc::sync_channel::<Either<(Resources,Vec<ffi::OsString>,Vec<(ffi::OsString,ffi::OsString)>,Vec<u8>,Vec<u8>,mpsc::SyncSender<Option<Pid>>,Option<usize>,Vec<net::SocketAddr>),(usize,Either<u16,u16>)>>(0);

	let mut nodes = nodes.into_iter().enumerate().map(|(i,(addr,(mem,cpu,bridges)))| {
		let node = Node{mem,cpu};
		let mut check_addresses = HashSet::new();
		let check_port = check_addresses.insert(addr); assert!(check_port);
		let (sender_a, receiver_a) = mpsc::sync_channel::<(Resources,Vec<ffi::OsString>,Vec<(ffi::OsString,ffi::OsString)>,Vec<u8>,Vec<u8>,Vec<net::SocketAddr>)>(0);
			...
			crossbeam::scope(|scope|{
				scope.spawn(||{
					for (process,args,vars,elf,arg,ports) in receiver {
						...
						bincode::serialize_into(&mut stream_write, &(elf.len() as u64)).unwrap();
				...
				scope.spawn(||{
					let sender = sender;
					while let Ok(done) = bincode::deserialize_from::<_,Either<u16,u16>>(&mut stream_read).map_err(map_bincode_err) {
		...
		for (bridge,ports) in bridges {
			for &port in &ports {
				let check_port = check_addresses.insert(port); assert!(check_port);
			}
			...
			thread::spawn(move||{
				...
				path.pop();
				path.push(&bridge);
				let mut file_in = fs::File::open(&path).or_else(|_|fs::File::open(&bridge)).unwrap_or_else(|_|panic!("Failed to open bridge {:?}", &bridge));
				let mut elf = Vec::new();
				file_in.read_to_end(&mut elf).unwrap();
				let (sender_, receiver) = mpsc::sync_channel::<Option<Pid>>(0);
				sender.send(Either::Left((Resources{mem:0,cpu:0.0},vec![ffi::OsString::from(bridge)],Vec::new(),elf,Vec::new(),sender_,Some(i),ports))).unwrap();
		...
		for stream in listener.incoming() {
			...
			thread::spawn(move||{
				...
				while let Ok((process,args,vars,elf,arg)) = parse_request(&mut stream_read) {
					// println!("parsed");
					let (sender_, receiver) = mpsc::sync_channel::<Option<Pid>>(0);
					sender.send(Either::Left((process,args,vars,elf,arg,sender_,None,vec![]))).unwrap();
					let pid: Option<Pid> = receiver.recv().unwrap();
					// let mut stream_write = stream_write.write();
	...
	for msg in receiver.iter() {
		match msg {
			Either::Left((process,args,vars,elf,arg,sender,force,ports)) => {
				...
				} else { Some(force.unwrap()) };
				if let Some(node) = node {
					let node = &mut nodes[node];
					node.1.alloc(&process);

					let mut sched_arg = Vec::new();
					bincode::serialize_into(&mut sched_arg, &SchedulerArg{scheduler:net::SocketAddr::new(node.3,addr.port())}).unwrap();
					sched_arg.extend(arg);
					node.0.send((process,args,vars,elf,sched_arg,ports)).unwrap();
					node.4.push_back((sender,process));
				} else {
					println!("Failing a spawn! Cannot allocate process {:#?} to nodes {:#?}", process, nodes);
			...
			Either::Right((node_,Either::Left(init))) => {
				...
				let x = processes.insert((node_,init), process); assert!(x.is_none());
